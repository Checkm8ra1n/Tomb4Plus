...
#define READ_JSON_ARGB(value_name, json, my_struct) { \
    ...
    if (r && JSON_INTEGER == json_getType(r)) { \
        uint8_t r8 = (uint8_t)json_getInteger(r); \
    ...
    if (g && JSON_INTEGER == json_getType(g)) { \
        uint8_t g8 = (uint8_t)json_getInteger(g); \
    ...
    if (b && JSON_INTEGER == json_getType(b)) { \
        uint8_t b8 = (uint8_t)json_getInteger(b); \
    ...
    if (a && JSON_INTEGER == json_getType(a)) { \
        uint8_t a8 = (uint8_t)json_getInteger(a); \
	...
	if (backup_object_info_array) {
		...
		for (int32_t i = 0; i < NUMBER_OBJECTS; i++) {
			if (game_mod_config.level_info[level].objects_info.slot_override[i] != i) {
				...
				int32_t original_bone_index = current_object_info_array[i].bone_index;
				int16_t *original_frame_base = current_object_info_array[i].frame_base;
				int16_t original_anim_index = current_object_info_array[i].anim_index;
...
bool LoadGameModConfigFirstPass() {
	char* json_buf = NULL;
	if (T4PLoadFileAtRelativePath("game_mod_config.json", &json_buf) <= 0) {
...
extern void T4PlusInit() {
	global_string_table = (char **)SYSTEM_MALLOC(MAX_T4PLUS_STRINGS * sizeof(int8_t*));
