...
int32_t* bones;
int32_t level_items;
int16_t number_rooms;

int16_t* OutsideRoomOffsets;
int8_t* OutsideRoomTable;
int16_t IsRoomOutsideNo;

MESH_INFO* SmashedMesh[16];
int16_t SmashedMeshRoom[16];
int16_t SmashedMeshCount = 0;
...
int16_t ItemNewRooms[MAX_ROOMS][2];
int16_t ItemNewRoomNo = 0;

uint8_t CurrentAtmosphere;
bool IsAtmospherePlaying;
int8_t cd_flags[128];

uint32_t FmvSceneTriggered;
uint32_t CutSceneTriggered;
bool SetDebounce;
int32_t framecount = 0;
int32_t reset_flag = 0;
int32_t WeaponDelay = 0;
int32_t LaserSightX;
int32_t LaserSightY;
int32_t LaserSightZ;
...
int16_t FXType;
int8_t PoisonFlag;
int8_t TriggerTimer = 0;
int8_t LaserSightActive = 0;
int8_t DeathMenuActive;

static PHD_VECTOR ClosestCoord;
static int32_t ClosestItem;
static int32_t ClosestDist;

static int32_t number_los_rooms = 0;
static int16_t los_rooms[20];

static int16_t cdtrack = -1;

static int32_t S_Death() {
	int32_t selection, menu, ret;
...
int32_t ControlPhase(int32_t nframes, int32_t demo_mode) {
	...
	for (framecount += nframes; framecount > 0; framecount -= 2) {
		...
		if (input & IN_LOOK && (lara_item->current_anim_state == AS_STOP && lara_item->anim_number == LARA_ANIM_BREATH ||
		                        (lara.IsDucked && !(input & IN_DUCK) && lara_item->anim_number == LARA_ANIM_DUCKBREATHE && lara_item->goal_anim_state == AS_DUCK))) {
			if (!BinocularRange) {
...
void FlipMap(int32_t FlipNumber) {
	...
	for (int32_t i = 0; i < number_rooms; i++) {
		...
		if (r->flipped_room >= 0 && r->FlipNumber == FlipNumber) {
			for (int32_t j = r->item_number; j != NO_ITEM; j = items[j].next_item) {
				items[j].il.room_number = 0xff;
			}
...
void RemoveRoomFlipItems(ROOM_INFO* r) {
	...
	for (int16_t item_num = r->item_number; item_num != NO_ITEM; item_num = item->next_item) {
		item = &items[item_num];
...
void TestTriggers(int16_t* data, bool heavy, int32_t heavy_flags) {
	...
	int32_t switch_off, flip, flip_available, neweffect, key, quad;
	int16_t camera_flags, camera_timer, type, trigger, value, flags, state;
	int8_t timer;
	...
	if ((*data & 0x1F) == CLIMB_TYPE) {
		if (!heavy) {
			quad = uint16_t(lara_item->pos.y_rot + (BLOCK_SIZE * 8)) >> W2V_SHIFT;
		...
		switch (type) {
			...
			case MONKEY:
				if (NGIsUsingNGNewTriggers()) {
					...
					int32_t result = NGRunConditionTrigger((uint16_t *)data-1);
	...
	if (NGIsUsingNGTimerfields()) {
		timer = (int8_t)(NGCalculateTriggerTimer(data, timer) & 0xff);
	}
...
int32_t GetHeight(FLOOR_INFO* floor, int32_t x, int32_t y, int32_t z) {
	...
	int32_t height;
	uint16_t trigger;
	int16_t type, dx, dz, xoff, yoff, tilt, hadj, tilt0, tilt1, tilt2, tilt3;
	...
	do {
		...
		switch (type & 0x1F) {
			...
			case TRIGGER_TYPE: {
				...
				int16_t trigger_type = (type >> 8) & 0x3F;
				int16_t flags = *data++;
				uint8_t timer = flags & 0xff;
...
int32_t GetCeiling(FLOOR_INFO* floor, int32_t x, int32_t y, int32_t z) {
	...
	ROOM_INFO* r;
	FLOOR_INFO* f;
	int32_t xoff, yoff, height, h1, h2;
	int16_t* data, type, trigger, dx, dz, t0, t1, t2, t3, hadj, ended;
...
void AlterFloorHeight(ITEM_INFO* item, int32_t height) {
	...
	if (floor->floor == -127) {
		floor->floor = ceiling->ceiling + int8_t(height >> 8);
	} else {
		floor->floor += int8_t(height >> 8);
...
void InterpolateAngle(int16_t dest, int16_t* src, int16_t* diff, int16_t speed) {
	...
	adiff = (uint16_t)dest - (uint16_t)*src;
	...
	if (diff) {
		diff[0] = (int16_t)adiff;
	}

	*src += int16_t(adiff >> speed);
}
...
void KillMoveEffects() {
	for (int32_t i = 0; i < ItemNewRoomNo; i++) {
...
void RefreshCamera(int16_t type, int16_t* data) {
	int16_t trigger, value, target_ok;
...
void TriggerNormalCDTrack(int16_t value, int16_t flags, int16_t type) {
	...
	if (value >= audio_info->first_looped_audio_track && value <= audio_info->last_looped_audio_track || value == 102 || value == 97) {
		if (CurrentAtmosphere != value) {
			CurrentAtmosphere = (uint8_t)value;
...
int32_t xLOS(GAME_VECTOR* start, GAME_VECTOR* target) {
	FLOOR_INFO* floor;
	int32_t dx, dy, dz, x, y, z;
	int16_t room_number, last_room;
	...
	if (dx < 0) {
		...
		y = ((x - start->x) * dy >> WALL_SHIFT) + start->y;
		z = ((x - start->x) * dz >> WALL_SHIFT) + start->z;
...
int32_t zLOS(GAME_VECTOR* start, GAME_VECTOR* target) {
	...
	if (dz < 0) {
		...
		x = ((z - start->z) * dx >> WALL_SHIFT) + start->x;
		y = ((z - start->z) * dy >> WALL_SHIFT) + start->y;
...
int32_t ExplodeItemNode(ITEM_INFO* item, int32_t Node, int32_t NoXZVel, int32_t bits) {
	...
	ShatterObject(&ShatterItem, 0, (int16_t)bits, item->room_number, NoXZVel);
...
int32_t IsRoomOutside(int32_t x, int32_t y, int32_t z) {
	...
	if (offset < 0) {
		...
		pTable = (uint8_t*)&OutsideRoomTable[offset];
...
int32_t ObjectOnLOS2(GAME_VECTOR* start, GAME_VECTOR* target, PHD_VECTOR* Coord, MESH_INFO** StaticMesh) {
	...
	for (int32_t i = 0; i < number_los_rooms; i++) {
		r = &room[los_rooms[i]];
...
int32_t GetTargetOnLOS(GAME_VECTOR* src, GAME_VECTOR* dest, int32_t DrawTarget, int32_t firing) {
	...
	GAME_VECTOR target;
	PHD_VECTOR v;
	int16_t item_no, hit, ricochet, room_number, TriggerItems[8], NumTrigs;

	hit = 0;
	target.x = dest->x;
	target.y = dest->y;
	target.z = dest->z;
	ricochet = (int16_t)LOS(src, &target);
	GetFloor(target.x, target.y, target.z, &target.room_number);
	...
	item_no = (int16_t)ObjectOnLOS2(src, dest, &v, &Mesh);
					...
					if (shotitem->object_number != SWITCH_TYPE7 && shotitem->object_number != SWITCH_TYPE8) {
						...
						if (ShatterItem.Bit == 1 << (objects[shotitem->object_number].nmeshes - 1) && !(shotitem->flags & IFL_SWITCH_ONESHOT)) {
							...
							if (shotitem->flags & IFL_CODEBITS && (shotitem->flags & IFL_CODEBITS) != IFL_CODEBITS) {
								...
								NumTrigs = (int16_t)GetSwitchTrigger(shotitem, TriggerItems, 1);

								for (int32_t i = NumTrigs - 1; i >= 0; i--) {
...
void AnimateItem(ITEM_INFO* item) {
	...
	if (item->frame_number > anim->frame_end) {
		if (anim->number_commands > 0) {
			...
			for (int32_t i = anim->number_commands; i > 0; i--) {
				switch (*cmd++) {
					case ACMD_SETPOS:
						// TRNG
	...
	if (anim->number_commands > 0) {
		...
		for (int32_t i = anim->number_commands; i > 0; i--) {
			switch (*cmd++) {
				case ACMD_SETPOS: {
					int32_t offset_frame = item->frame_number - anim->frame_base;
				...
				case ACMD_PLAYSFX:
					...
					if (item->frame_number == *cmd) {
						...
						if (objects[item->object_number].water_creature) {
							...
							item->pos.y_pos = lara_item->pos.y_pos - LARA_STANDARD_HEIGHT;
...
int32_t RayBoxIntersect(PHD_VECTOR* min, PHD_VECTOR* max, PHD_VECTOR* origin, PHD_VECTOR* dir, PHD_VECTOR* Coord) {
#define MAX_PLANES 3
	int32_t planes[MAX_PLANES];
	int32_t dists[MAX_PLANES];
	int32_t plane;
	int8_t quad[MAX_PLANES];
	int8_t inside;
	...
	for (int32_t i = 1; i < MAX_PLANES; i++) {
...
int32_t DoRayBox(GAME_VECTOR* start, GAME_VECTOR* target, int16_t* bounds, PHD_3DPOS* ItemPos, PHD_VECTOR* Coord, int16_t item_number) {
	...
	int16_t** meshpp;
	int16_t* ClosestMesh;
	int32_t x, y, z, ClosestNode, ClosestBit, bit, r, r0, r1, dist, max_dist;
	...
	tpos.x = int32_t(mMXPtr[M00] * x + mMXPtr[M01] * y + mMXPtr[M02] * z);
	tpos.y = int32_t(mMXPtr[M10] * x + mMXPtr[M11] * y + mMXPtr[M12] * z);
	tpos.z = int32_t(mMXPtr[M20] * x + mMXPtr[M21] * y + mMXPtr[M22] * z);

	x = start->x - ItemPos->x_pos;
	y = start->y - ItemPos->y_pos;
	z = start->z - ItemPos->z_pos;
	spos.x = int32_t(mMXPtr[M00] * x + mMXPtr[M01] * y + mMXPtr[M02] * z);
	spos.y = int32_t(mMXPtr[M10] * x + mMXPtr[M11] * y + mMXPtr[M12] * z);
	spos.z = int32_t(mMXPtr[M20] * x + mMXPtr[M21] * y + mMXPtr[M22] * z);
	...
	x = int32_t(mMXPtr[M00] * Coord->x + mMXPtr[M01] * Coord->y + mMXPtr[M02] * Coord->z);
	y = int32_t(mMXPtr[M10] * Coord->x + mMXPtr[M11] * Coord->y + mMXPtr[M12] * Coord->z);
	z = int32_t(mMXPtr[M20] * Coord->x + mMXPtr[M21] * Coord->y + mMXPtr[M22] * Coord->z);
	Coord->x = x;
	Coord->y = y;
	Coord->z = z;
	...
	if (item_number < 0) {
		...
		for (int32_t i = 0; i < obj->nmeshes; i++, meshpp += 2, bit <<= 1) {
...
int32_t GetMaximumFloor(FLOOR_INFO* floor, int32_t x, int32_t z) {
	int32_t height, h1, h2;
	int16_t* data, type, dx, dz, t0, t1, t2, t3, hadj;
...
int32_t GetMinimumCeiling(FLOOR_INFO* floor, int32_t x, int32_t z) {
	int32_t height, h1, h2;
	int16_t* data, type, dx, dz, t0, t1, t2, t3, hadj, ended;
